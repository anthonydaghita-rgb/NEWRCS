...
interface ServicePageProps {
  service: ServiceData
  relatedServicesData: ServiceData[]
}

export default function ServicePage({ service, relatedServicesData }: ServicePageProps) {
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {relatedServicesData.map((related) => {
                const RelatedIcon = iconMap[related.iconName] || Monitor
                return (
                  <ServiceCard
                    key={related.id}
                    title={related.title}
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  // Serialize the service object (convert icon to name string)
  const serializedService: ServiceData = {
    id: service.id,
    title: service.title,
    shortTitle: service.shortTitle,
    description: service.description,
    iconName: service.icon.name || 'Monitor',
    slug: service.slug,
  ...
  const serializedRelated: ServiceData[] = relatedServices.map(s => ({
    id: s.id,
    title: s.title,
  ...
  return {
    props: {
      ...
      relatedServicesData: serializedRelated,
    },
  }
}
